...
def train(model, train_loader, train_images, e, obs_len, pred_len, batch_size, params, gt_template, device, input_template, optimizer, criterion, dataset_name, homo_mat):
	...
	for batch, (trajectory, meta, scene) in enumerate(train_loader):
		...
		for i in range(0, len(trajectory), batch_size):
			...
			print(gt_future_map.shape)
			print(gt_waypoint_map.shape)
			print()
...
def train_workzone(model, train_loader, epoch, obs_len, pred_len, batch_size, params, gt_template, device, input_template, optimizer, criterion, dataset_name):
	train_loss = 0
	train_ADE = []
	train_FDE = []
	model.train()
	counter = 0
	# outer loop, for loop over each scene as scenes have different image size and to calculate segmentation only once
	for batch, (img, trajectory, meta) in enumerate(train_loader):
		...
		gt_waypoints = gt_future[:, :, params['waypoints'], :].to(device)
		gt_waypoint_map = get_patch(input_template, gt_waypoints.reshape(-1, 2).cpu().numpy(), H, W)
		gt_waypoint_map = torch.stack(gt_waypoint_map).reshape([-1, gt_waypoints.shape[2], H, W])
		...
		with torch.no_grad():
			...
			train_ADE.append(((((gt_future - pred_traj)) ** 2).sum(dim=2) ** 0.5).mean(dim=1))
			train_FDE.append(((((gt_future[:, -1:] - pred_goal[:, -1:])) ** 2).sum(dim=2) ** 0.5).mean(dim=1))
